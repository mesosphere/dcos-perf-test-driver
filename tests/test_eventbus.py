...
class TestEventBus(unittest.TestCase):
  ...
  def test_event(self):
    """
    Test if event construction works as expected
    """
    event = Event(traceid=['foo', 'bar'])
    self.assertIn('foo', event.traceids)
    self.assertIn('bar', event.traceids)
