...
MARATHON_SUCCESS_EVENT = re.compile(r'^.+Successfully started.*?[1-9][0-9]*.*?(\/[^ ]+).*$')
MARATHON_DEPLOYMENT_COMPLETE = re.compile(r'^.*Removing ([-\w]+) from list of running deployments.*$')
...
class MarathonLogsObserver(Observer):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.eventbus.subscribe(self.handleHTTPRequestStart, events=(HTTPRequestStartEvent,))
  ...
  def handleLogLine(self, event):
    ...
    if match:
      ...
      if instance in self.instanceTraceIDs:
        ...
        self.logger.warn('Found a deployment success event without an http request')
      return
    ...
    if match:
      ...
      self.eventbus.publish(
        MarathonDeploymentSuccessEvent(deployment, {})
      )
