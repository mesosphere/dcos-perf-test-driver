...
class MarathonUpdateChannel(Channel):
  ...
  def handleUpdate_PatchApp(self, action, parameters):
    ...
    try:
      ...
      # Query all items
      response = requests.get('%s/v2/apps' % (self.url,), verify=False, headers=self.getHeaders())
    ...
    for app in apps:
      try:
        ...
        self.logger.debug('Patching %i with %r' % (app['id'], patch))
        app.update(patch)

        # Update the specified application
        self.logger.debug('Executing update')
        response = requests.put('%s/v2/apps%s' % (self.url, app['id']), body=app, verify=False, headers=self.getHeaders())
