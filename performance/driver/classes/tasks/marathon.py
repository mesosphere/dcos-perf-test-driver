...
class RemoveAllApps(Task):
  ...
  def run(self):
    ...
    response = requests.get('%s/service/marathon/v2/groups?_timestamp=1494604512847&embed=group.groups&embed=group.apps&embed=group.pods&embed=group.apps.deployments&embed=group.apps.counts&embed=group.apps.tasks&embed=group.apps.taskStats&embed=group.apps.lastTaskFailure' % cluster_url, verify=False, headers=headers)
    ...
    for app in response.json()['apps']:
      ...
      response = requests.delete('%s/service/marathon/v2/apps/%s' % (cluster_url, app['id']), verify=False, headers=headers)
      if response.status_code != 200:
